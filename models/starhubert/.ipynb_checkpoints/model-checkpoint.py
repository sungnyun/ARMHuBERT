...
class CustomStudentModelConfig(FairseqDataclass):
    ...
    pred_head_final_dim: int = field(
        ...
        metadata={"help": "Final output dimension of prediction head"
                          "Must be same as transformer hidden dimension of teacherm model"}
    )
...
class CustomStudentModel(BaseFairseqModel):
        ...
        **kwargs,
        ...
        if cfg.layerwise_proj:
            ...
            self.proj_head = nn.ModuleList([
                ...
                ) for _ in range(cfg.encoder_layers)  ## Including 0th input proj head
            ])
            self.tr_proj_head = LayerWiseProjHead(
                    in_dim=cfg.encoder_embed_dim,
                    out_dim=cfg.pred_head_final_dim,
                    enable_tr_layer=cfg.enable_tr_layer,
                    tr_reduce_factor=cfg.tr_reduce_factor,
    ...
    ## Call this when FSP is not necessary
    def _disable_tr_projection_head(self):
        self.tr_proj_head = None
    ...
    def forward(
        ...
        if self.layerwise_proj:
            if self.proj_head:
                ...
                x = projections[-1]
                projections.insert(-1, self.tr_proj_head(tr_layer_results))
            else:
                x = self.final_proj(x)
                projections = None

        else:  # if not layerwise projection
