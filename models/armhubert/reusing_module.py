...
## Need to be erased after merge to module.py
from .module import TransformerSentenceEncoderLayer
...
class MultiheadAttentionReuse(nn.Module):
    """Multi-headed attention with reusing attention map.
    ...
    def __init__(
        ...
        self.bias_v = Parameter(torch.Tensor(1, 1, embed_dim)) if add_bias_v else None
    ...
    def _add_bias(
        ...
        return v, key_padding_mask, attn_mask
    ...
    def forward(
        ...
        Args:
            attn_map: attention map of previous Transformer layers,
                of shape `(# heads, bsz, src_len, src_len)`
        ...
        """
        # Needs to utilize attention map & value
        ...
        if self.bias_v is not None:
            v, key_padding_mask, attn_mask = self._add_bias(
                v, key_padding_mask, attn_mask, bsz
            )
        ...
        #     #attn_weights =  # Previous layer attention weights
        
        # Attention map reuse
        # View as bsz, # of head
        attn_weights = attn_map.contiguous().view(bsz * self.num_heads, src_len, src_len)
        ...
        # attn: T B D, attn_weights: Head B T T
...
class TransformerSentenceEncoderLayerReuse(nn.Module):
    """
    Implements a Transformer Encoder Layer used in BERT/XLM style pre-trained
    models with attention map reuse
    """
    ...
    def __init__(
        ...
        self.self_attn = MultiheadAttentionReuse(
            self.embedding_dim,
            num_attention_heads,
    ...
    def forward(
        ...
        attn_map: torch.Tensor, # attention map from previous layer (# heads * bsz, src_len, src_len)
...
class TransformerEncoderWithReuse(nn.Module):
        ...
        if args.layer_type == "transformer":
            layer = [TransformerSentenceEncoderLayer(
                ...
                layer_norm_first=args.layer_norm_first,
                    ...
                    TransformerSentenceEncoderLayerReuse(
                embedding_dim=self.embedding_dim,
                ffn_embedding_dim=args.encoder_ffn_embed_dim,
                num_attention_heads=args.encoder_attention_heads,
                dropout=self.dropout,
                attention_dropout=0.0, ## 0 in original paper
                activation_dropout=args.activation_dropout,
                activation_fn=args.activation_fn,
                layer_norm_first=args.layer_norm_first,                
            )
    ...
    def __init__(self, args):
        ...
        ## Attention map reuse only, # of attention heads are fixed 
        reuse_pattern = eval(args.reuse_pattern)
        num_layers = args.encoder_layers
        self.reuse_pattern = reuse_pattern
        self.num_encoder_layers = num_layers

        if reuse_pattern is None:
            reuse_pattern = tuple([True] * num_layers)
        if len(reuse_pattern) != num_layers:
            raise ValueError(f"Sharing pattern length {len(reuse_pattern)} mismatch to num_layers {num_layers}")
        if not reuse_pattern[0]:
            raise ValueError("First layer pattern should be True")

        layers = []
        for rp in reuse_pattern:
            layer = self.build_encoder_layer(args)[0] if rp else self.build_encoder_layer(args)[1]
            layers.append(layer)
        self.layers = nn.ModuleList(layers)

        if args.enable_tr_layer:
            self.layers.insert(args.tr_layer_index, tr_layer)
            self.reuse_pattern.insert(args.tr_layer_index, None)  ## Need to more developed for nonzero tr_layer_index
    ...
    def extract_features(
        ...
        tr_layer_results = []
        r = None
        reused_attn_map = None

        ## Need to change
        for i, (layer, rp) in enumerate(zip(self.layers, self.reuse_pattern)):
            dropout_probability = np.random.random() if self.layerdrop > 0 else 1
            if not self.training or (dropout_probability > self.layerdrop):
                if isinstance(layer, nn.Linear):
                    assert self.reuse_pattern[i+1] == True  ## Attention map must be created after tr layer
                    ...
                    assert self.reuse_pattern[i+1] == True
                    ...
                    if rp:
                        x, (z, lr) = layer(  # x, attention_weights, layer_results
                            x, self_attn_padding_mask=padding_mask, need_weights=False # need_head_weights is True
                        )
                        reused_attn_map = z
                    else:
                        assert reused_attn_map is not None
                        x, (z, lr) = layer(
                            x, reused_attn_map, self_attn_padding_mask=padding_mask, need_weights=False
                        )
                    ####################### Gradient scale ####################### 
        ...
        if pad_length > 0:
            ...
            # We only need x for layer result, therefore only pad 0th index
            layer_results = [undo_pad(*u)[0] for u in layer_results]
        # Handle for else loop
        else:
